...
import { Target, Crosshair, AlertCircle, ChevronDown, Bot, Loader } from "lucide-react";
...
export default function MatchDifferPage() {
  ...
  function mdOnUpdate(upd: Partial<TradeResult> & { id: string }) {
    setMdTrades(prev => {
  ...
  // Auto-fire MATCH — 2-sec cooloff between fires
  useEffect(() => {
    if (!aiMatchOn || deriv.status !== "connected" || mdExecRef.current) return;
    if (!matchFire || matchConf < 60) return;
    const now = Date.now();
    if (now - mdMatchCoolRef.current < 2000) return;
    const key = `m-${bestMatch}-${matchConf.toFixed(0)}-${matchStrategy}`;
  ...
  }, [aiMatchOn, deriv.status, matchFire, matchConf, bestMatch, matchStrategy, mdStake, mdMartOn, mdMartMult, mdLossStreak, symbol]);

  // Auto-fire DIFFER — 2-sec cooloff between fires
  useEffect(() => {
    if (!aiDifferOn || deriv.status !== "connected" || mdExecRef.current) return;
    if (!differFire || differConf < 65) return;
    const now = Date.now();
    if (now - mdDifferCoolRef.current < 2000) return;
    const key = `d-${bestDiffer}-${differConf.toFixed(0)}-${differStrategy}`;
    ...
    setMdExecuting(true);
    const stake    = mdMartOn ? nextStake(mdStake, mdMartMult, mdLossStreak) : mdStake;
    const aiTicks  = differTicks > 0 ? differTicks : 1;
    const currency = deriv.account?.currency ?? "USD";
    const specs: TradeSpec[] = [{
      contract_type: "DIGITDIFF", symbol, stake, ticks: aiTicks,
      digit: bestDiffer, label: `DIFFERS D${bestDiffer} ${aiTicks}T @ ${differConf.toFixed(0)}%`,
    ...
    void executeBulk(specs, deriv.request, deriv.subscribe, currency, mdOnUpdate)
      ...
      .finally(() => { mdExecRef.current = false; setMdExecuting(false); });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [aiDifferOn, deriv.status, differFire, differConf, bestDiffer, differStrategy, mdStake, mdMartOn, mdMartMult, mdLossStreak, symbol]);
        ...
        {deriv.status !== "connected" && (aiMatchOn || aiDifferOn) && (
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg"
            ...
            <span className="font-rajdhani text-xs text-red-400">
              Connect Deriv account via the panel below to enable auto-trading.
            </span>
          </div>
        )}

        {/* Active signal status cards */}
        {(aiMatchOn || aiDifferOn) && deriv.status === "connected" && (
          <div className="space-y-2">
            {aiMatchOn && (
              <div className="p-3 rounded-xl space-y-1.5"
                style={{ background: matchFire ? "rgba(34,197,94,0.07)" : "rgba(255,255,255,0.02)", border: `1px solid ${matchFire ? "rgba(34,197,94,0.4)" : "rgba(255,255,255,0.07)"}` }}>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <div className="w-7 h-7 rounded-full flex items-center justify-center font-orbitron text-sm font-black text-white flex-shrink-0"
                    style={{ background: DIGIT_COLORS[bestMatch], boxShadow: `0 0 10px ${DIGIT_COLORS[bestMatch]}60` }}>
                    {bestMatch}
                  </div>
                  <span className="font-rajdhani text-[10px] text-muted-foreground">AI BARRIER: Digit {bestMatch}</span>
                  <span className="font-rajdhani text-[10px] text-muted-foreground">· AI TICKS: {matchTicks || 1}T</span>
                  <span className="ml-auto font-orbitron text-xs font-bold"
                    style={{ color: matchConf >= 70 ? "#22c55e" : matchConf >= 55 ? "#facc15" : "#ef4444" }}>
                    {matchConf.toFixed(0)}%
                  </span>
                  {matchFire ? (
                ...
                {matchStrategy && (
                  <div className="font-rajdhani text-[9px] font-bold" style={{ color: "#00e5ff" }}>{matchStrategy}</div>
                )}
                {mdExecuting && (
            ...
            {aiDifferOn && (
              <div className="p-3 rounded-xl space-y-1.5"
                style={{ background: differFire ? "rgba(239,68,68,0.07)" : "rgba(255,255,255,0.02)", border: `1px solid ${differFire ? "rgba(239,68,68,0.4)" : "rgba(255,255,255,0.07)"}` }}>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="font-orbitron text-[10px] font-bold tracking-widest" style={{ color: "#ef4444" }}>DIRECTION: DIFFERS</span>
                  <div className="w-7 h-7 rounded-full flex items-center justify-center font-orbitron text-sm font-black text-white flex-shrink-0"
                    style={{ background: DIGIT_COLORS[bestDiffer], boxShadow: `0 0 10px ${DIGIT_COLORS[bestDiffer]}60` }}>
                    {bestDiffer}
                  </div>
                  <span className="font-rajdhani text-[10px] text-muted-foreground">AI BARRIER: Digit {bestDiffer}</span>
                  <span className="font-rajdhani text-[10px] text-muted-foreground">· AI TICKS: {differTicks || 1}T</span>
                  <span className="ml-auto font-orbitron text-xs font-bold"
                    style={{ color: differConf >= 70 ? "#22c55e" : differConf >= 55 ? "#facc15" : "#ef4444" }}>
                    {differConf.toFixed(0)}%
                  </span>
                  {differFire ? (
                    <span className="px-1.5 py-0.5 rounded text-[9px] font-bold font-orbitron animate-pulse"
                ...
                {differStrategy && (
                  <div className="font-rajdhani text-[9px] font-bold" style={{ color: "#ef4444" }}>{differStrategy}</div>
                )}
                {mdExecuting && (
            ...
            {mdTrades.slice(0, 20).map((t, i) => (
              <div key={t.id ?? String(i)} className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg"
                ...
                <span className="font-orbitron text-[10px] font-bold flex-shrink-0"
                  style={{ color: t.status === "won" ? "#22c55e" : t.status === "lost" ? "#ef4444" : t.status === "open" || t.status === "pending" ? "#facc15" : "#888" }}>
